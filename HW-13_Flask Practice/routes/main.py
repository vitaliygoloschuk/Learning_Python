...
def sign_up():
    if request.method == "POST":
        data = request.form
        email_usr = User.query.filter(User.email == request.form.get("email")).first()
        if email_usr is not None:
            ath_email = True
            return render_template("sign-up.html", ath_email=ath_email)
        elif data.get("password") == "":
            ath_pass = True
            return render_template("sign-up.html", ath_pass=ath_pass)
        elif data.get("username") == "":
            ath_user = True
            return render_template("sign-up.html", ath_user=ath_user)
        else:
            user = User(
